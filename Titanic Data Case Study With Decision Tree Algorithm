...
#Read the dataset----
titanic<-read.csv("C:/Users/91810/Desktop/RStudio files/Practice R files/titanic.csv",header = T)
...
View(titanic)
library(dplyr)
clean_titanic_dataset <- titanic %>%
  select(-c(home.dest, cabin, name, x, ticket)) %>% 
  #Convert to factor level
  mutate(pclass = factor(pclass, levels = c(1, 2, 3), labels = c('Upper', 'Middle', 'Lower')),
         sex = factor(sex, levels = c('female', 'male'),  labels = c('female', 'male')),
         embarked = factor(embarked, levels = c('C','Q','S'),  labels = c('C','Q','S')),
         survived = factor(survived, levels = c(0,1),  labels = c(0,1)))%>% 
na.omit(titanic)
glimpse(clean_titanic_dataset)
str(clean_titanic_dataset)

getwd()
setwd("C:/Users/91810/Desktop/RStudio files/Practice R files")
write.csv(clean_titanic_dataset,"titanic_02.csv")
#Convert few cont var from factor to numeric----
clean_titanic_dataset$fare <- as.numeric(as.character(clean_titanic_dataset$fare))
clean_titanic_dataset$age <- as.numeric(as.character(clean_titanic_dataset$age))

#Convert few Categorical var from chr to factor----
# clean_titanic_dataset$sex <- as.factor((clean_titanic_dataset$sex))
# clean_titanic_dataset$embarked <- as.factor((clean_titanic_dataset$embarked))

str(clean_titanic_dataset)
#Create train/test set----
View(clean_titanic_dataset)
trainDataIndex <- sample(1:nrow(clean_titanic_dataset),0.8*nrow(clean_titanic_dataset), replace = F)
trainData <-clean_titanic_dataset[trainDataIndex, ]
testData <- clean_titanic_dataset[-trainDataIndex, ]
View(trainData)
View(testData)
...
table(testData$survived)/nrow(testData)
